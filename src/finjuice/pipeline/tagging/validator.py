...
* **Conflict detection** — inspects already-loaded
  :class:`~finjuice.pipeline.tagging.models.TagRule` objects for pattern
  overlaps, priority inversions, duplicate names, and regex issues. The
  issue dataclasses, checks, and :func:`validate_rules` entrypoint live in
  :mod:`finjuice.pipeline.tagging.validator_conflicts` and are re-exported
...
from finjuice.pipeline.tagging.validator_conflicts import (
    ...
    ValidationResult,  # noqa: F401 — re-exported for CLI/test callers.
    _get_patterns,  # noqa: F401 — re-exported for tests.
    _is_broader_pattern,  # noqa: F401 — re-exported for tests.
    _patterns_overlap,  # noqa: F401 — re-exported for tests.
    check_duplicate_names,  # noqa: F401 — re-exported for CLI/test callers.
    check_pattern_overlaps,  # noqa: F401 — re-exported for tests.
    check_priority_inversions,  # noqa: F401 — re-exported for tests.
    check_regex_validity,  # noqa: F401 — re-exported for tests.
    validate_rules,  # noqa: F401 — re-exported for CLI callers.
)
