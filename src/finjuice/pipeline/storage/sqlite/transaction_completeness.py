"""Source-backed transaction completeness evidence and pure scope validation."""
...
def require_transaction_completeness(
    snapshot: TransactionReadSnapshot, *, month: str | None = None
) -> None:
    """Reject incomplete requested scope without querying or exposing source details."""
    if snapshot.unmaterialized_months and (
        month is None or month in snapshot.unmaterialized_months
    ):
        raise RepositoryIntegrityError("Preserved transaction rows lack typed read values.")
