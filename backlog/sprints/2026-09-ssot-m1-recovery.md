...
- [x] #431 feat(backup): 기존 데이터셋 전체 사본·검증·격리 복원 (implementation: PR #451; operational evidence: PR #452)

### Batch 3 — 장비 밖 복원 실증

- [x] #432 ops(backup): 장비 밖 백업·키 복구·첫 복원 실증 (operational evidence: PR #452)

### Batch 4 — M1 검증과 다음 단계 연결

- [x] #426 epic: SSOT 전환 계약과 전체 백업·복원

## Running Context
- 실행 계약은 `goals/finjuice-ssot.md`, 공통 맥락은 `_context.md`를 읽는다. 명세와 AC는 GitHub에서 최신 상태를 확인한다.
- #430·#431·#432와 M1 에픽 #426은 실제 AC 확인 후 COMPLETED로 닫았다. 전체 #425 목표는 계속된다.
- 기존 운영 서비스는 0.7.1이며 SQLite 운영 전환은 아직 하지 않았다. 백업 도구 0.7.2는 별도로 설치·검증했다.
- 첫 금융 기준선과 모든 대응 복구 절차 사본은 영구 보호한다. 과거 실패·성공 기록을 덮어쓰거나 정리하지 않는다.
- 복원에는 최신 검토 도구가 든 장비 밖 절차 사본을 사용한다. 원래 기준선에 포함된 옛 도구나 원격 캡처 계획을 수정 없이 재사용하지 않는다.
- M2 스프린트는 이 스프린트를 마친 후 생성한다. 최종 보존 검증과 운영 전환에는 해당 시점의 writer 통제 아래 새로 취득한 기준선이 필요하다.
- 사용자 원래 checkout의 변경은 보존한다. 다음 구현은 최신 main의 별도 codex branch에서 진행한다.

## Progress
- 현재 checkpoint: M1의 실제 캡처·Linux 격리 복원·장비 밖 독립 복원·최종 교차 검토·복구 절차 재복원을 통과했고 #431·#432·#426을 COMPLETED로 닫았다. 다음은 M2 #433이다. 이전 날짜의 미실행/검토 대기 항목은 당시 상태다.
...
- 2026-09-09: Claude Opus 교차 검토의 P2 두 건을 수정했다. 전체 조회는 명시적인 비절단 표시·행 수·전체 결과 수를 확인하고, 봉인된 과거 조회도 엄격히 재검증한다. 쓰기 프로세스 검사는 정규 경로를 사용하며 경로의 모든 symlink를 거부한다. 디렉터리 탐색 오류 전파와 단일 JSON 출력도 보완했다. 합성 검사는 Mac 8개 통과·Linux 전용 1개 제외, Linux 9개 모두 통과했다. 기존 캡처는 별도 사전 검사와 전체 파일 목록 대조에서 결함의 발동 조건이 없음을 확인했다.
- 2026-09-09: 보완된 복구 절차를 새 암호화 snapshot으로 보존하고, 원래 기준선과 함께 NAS에서 새로 내려받았다. 복원된 도구·프로그램만으로 모든 manifest entry와 대표 조회 4개를 재검증했다. 이번 추가 독립 시도는 약 51초였으며 원본 장비에 접근하지 않았다. 초기 기준선·이전 절차 사본·실패 증거는 모두 보존한다. 수정분의 최종 재검토를 진행 중이다.
- 2026-09-09: 전체 조회와 status 두 종류의 데이터셋 총 건수, 독립 원본 fingerprint와 백업 manifest의 전체 역할·경로·내용·메타데이터를 자동 대조하도록 보완했다. 잘못된 역할 이름·미승인 계획도 캡처 전에 거부한다. Linux 합성 검사 12개와 최신 도구의 새 NAS 독립 복원이 통과했다(약 45초). 운영 서비스는 기존 0.7.1로 정상 동작함을 재확인했다. 최종 코드 교차 검토와 완료 기록 보존만 남아 있다.
- 2026-09-09: Claude Opus 5 high 최종 검토에서 남은 P1/P2 없음. 검토 도구의 정확한 해시가 실제 복원본과 일치하며 전체 조회·독립 원본 목록의 대응도 통과했다. 최종 검토·절차·근거를 새 암호화 snapshot에 보존하고 재복원한 파일 해시를 검증했다. #431·#432·#426의 실제 AC를 체크하고 COMPLETED로 닫았다. M1을 마감하고 M2로 이어간다.
- 2026-09-09: Sprint closed. 4/4 tasks completed.
