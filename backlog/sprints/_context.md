...
- 첫 활성 실행 계획: `backlog/sprints/2026-09-ssot-m1-recovery.md`. #430은 #449·#450, 백업 구현은 #451로 머지됐다. 현재 운영 검증 기록 branch는 `codex/ssot-m1-operations`이며 `git worktree list`로 해당 작업 공간을 확인한다. 실제 동결 캡처·Linux 격리 복원·암호화 장비 밖 사본의 독립 복원은 통과했고, 운영 보완의 최종 교차 검토와 기록 보존 후 M1을 닫는다.
