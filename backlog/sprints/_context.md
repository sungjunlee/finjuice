...
- 2026-09-13 현재 #435 작업은 PR #481 / `codex/ssot-m2-migrate`다. main 0.8.2를 `5297c05`로 통합했고 전체 3,303 PASS·1 SKIP를 확인했다. canonical rules/goals head 선택은 `09c8247`에 구현했다. 이어 새 v3 계획은 기존 CSV 해석기의 공백·중복 marker 선택 의미를 재현하면서 원문/visible 순서·중복/저장된 최종 분류를 보존한다. 기존 v1/v2 후보는 원래 정책으로 재생하며 v3도 canonical 설정 선택을 유지한다. 교차 파일 ADR-0015, 장애 lineage, consumer parity는 후속 수용 조건이다. #463의 필수 GitHub 승인을 대체하지 않는다.
